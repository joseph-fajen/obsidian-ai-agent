...
**Good Divergence** = The implementation correctly adapted when the plan was flawed.
- The plan assumed something that didn't exist (component, pattern, API)
- The plan's technical approach wouldn't work in practice
- A better pattern was discovered that the plan didn't anticipate
- A security or performance issue required a different approach
- **Key question:** Was diverging the right call? Yes → Good divergence
- **System improvement:** Update planning process (better research, verify assumptions)

**Bad Divergence** = The implementation deviated when the plan was correct.
- The plan's constraints were ignored or overlooked
- New architecture was created instead of following existing patterns
- Shortcuts were taken that introduce technical debt
- Requirements were misunderstood despite clear plan language
- **Key question:** Was diverging the right call? No → Bad divergence
- **System improvement:** Update execution discipline (CLAUDE.md constraints, execute command)

**Repeated Issues** = The same divergence type appears across multiple implementations.
- This reveals missing automation or undocumented patterns
- **System improvement:** Create new commands or add to CLAUDE.md
...
justified: yes/no
root_cause: [unclear plan | missing context | missing validation | repeated manual step | N/A]
```
...
- **Suggest actual text:** Don't just recommend updates - draft the actual text to add to CLAUDE.md or commands
